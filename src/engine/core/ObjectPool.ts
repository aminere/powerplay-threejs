...
export class ObjectPool<T extends object> {
    ...
    get(amount: number) {
        ...
        this._size = newSize;
        return this._objects.slice(offset, this._size);
    }
    ...
    private grow() {
        console.warn(`${this._objects[0].constructor.name} pool capacity reached, doubling the pool. Consider allocating a bigger pool.`);
